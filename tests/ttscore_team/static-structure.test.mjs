...
test("HTML подключает общие assets версии 0.11.1", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  assert.match(html, /\.\/assets\/0\.11\.1\/styles\.css/);
  assert.match(html, /\.\/assets\/0\.11\.1\/app\.mjs/);
...
test("переименование matches-source выполнено полностью", () => {
  assert.equal(existsSync(new URL("team/assets/0.11.1/matches-source.mjs", root)), true);
  assert.equal(existsSync(new URL("team/assets/0.11.1/meeting-source.mjs", root)), false);
  assert.match(read("team/assets/0.11.1/app.mjs"), /\.\/matches-source\.mjs/);
  assert.doesNotMatch(read("team/assets/0.11.1/app.mjs"), /meeting-source/);
});

test("Firebase остаётся оперативным источником, GitHub JSON используется только как public read-only fallback", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const firebase = read("team/assets/0.11.1/firebase-source.mjs");
  const archive = read("team/assets/0.11.1/archive-source.mjs");
...
test("относительные reportUrl по-прежнему разрешаются от team/matches/<id>/", () => {
  const source = read("team/assets/0.11.1/matches-source.mjs");
  assert.match(source, /matches\/\$\{id\}\//);
  assert.match(source, /teamMatchLinkedResourceUrl/);
  assert.match(read("team/assets/0.11.1/app.mjs"), /publishedLinkHref/);
});

test("данные примера отсутствуют в общем исходном коде", () => {
  const source = [
    read("team/ttscore_team_0.11.1.html"),
    read("team/assets/0.11.1/app.mjs"),
    read("team/assets/0.11.1/creator.mjs"),
    read("team/assets/0.11.1/editor.mjs"),
    read("team/assets/0.11.1/file-save.mjs"),
    read("team/assets/0.11.1/firebase-source.mjs"),
    read("team/assets/0.11.1/archive-source.mjs"),
    read("team/assets/0.11.1/model.mjs"),
    read("team/assets/0.11.1/matches-source.mjs"),
    read("team/assets/0.11.1/ui-state.mjs"),
    read("team/assets/0.11.1/ttscore-integration.mjs"),
    read("team/assets/0.11.1/styles.css")
  ].join("\n");
...
test("публичная страница не предлагает переход в редактор", () => {
  ...
  assert.match(read("team/assets/0.11.1/app.mjs"), /request\.mode !== "edit"/);
});
...
test("Team-level Undo отделён от operational transition и требует preview перед публикацией", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const editor = read("team/assets/0.11.1/editor.mjs");
  const contract = read("team/assets/0.11.1/team-integration-contract.mjs");
...
test("одно адаптивное сохранение выбирает picker, Web Share и только затем Blob", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const saver = read("team/assets/0.11.1/file-save.mjs");
...
test("v0.11.1 сворачивает все рабочие разделы edit кроме завершения текущей встречи", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
...
test("v0.11.1 показывает операционные Live-табло и Live-отчёт и использует названия команд в итоговом счёте", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const integration = read("team/assets/0.11.1/ttscore-integration.mjs");
...
test("редактор выделяет одну встречу нажатием и после перемещения", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const styles = read("team/assets/0.11.1/styles.css");
...
test("режим create подключён без GitHub API", () => {
  ...
  assert.match(read("team/ttscore_team_0.11.1.html"), /id="creator-id-warning"/);
...
test("создатель загружает локальный JSON без GitHub API и миграции schemaVersion 3", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const creator = read("team/assets/0.11.1/creator.mjs");
...
test("v0.11.1 сохраняет локальную интеграцию ttScore без backend", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const integration = read("team/assets/0.11.1/ttscore-integration.mjs");
  assert.equal(existsSync(new URL("team/assets/0.11.1/ttscore-integration.mjs", root)), true);
  for (const id of [
...
test("v0.11.1 не создаёт и не редактирует liveUrl в командном JSON", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const creator = read("team/assets/0.11.1/creator.mjs");
  const editor = read("team/assets/0.11.1/editor.mjs");
...
test("v0.11.1 публикует Live-отчёт и Live-табло текущей встречи из верхнего уровня JSON", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const creator = read("team/assets/0.11.1/creator.mjs");
  const editor = read("team/assets/0.11.1/editor.mjs");
...
test("v0.11.1 использует Firebase как опубликованный источник и realtime для public view", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const firebase = read("team/assets/0.11.1/firebase-source.mjs");
  for (const id of [
...
test("v0.11.1 Firebase-editor подписан на внешние realtime updates и защищает локальный draft", () => {
  const app = read("team/assets/0.11.1/app.mjs");
  assert.match(app, /startFirebaseRealtimeEditor/);
  assert.match(app, /applyRealtimeEditorData/);
...
test("v0.11.1 после запуска следующей ttScore-встречи не перезаписывает ручную корректировку pending-результата событиями live", () => {
  const app = read("team/assets/0.11.1/app.mjs");
...
test("v0.11.1 автоматически публикует только подтверждённые ttScore-переходы и операционные Live-ссылки", () => {
  const app = read("team/assets/0.11.1/app.mjs");
  const integration = read("team/assets/0.11.1/ttscore-integration.mjs");
  const editor = read("team/assets/0.11.1/editor.mjs");
  const contract = read("team/assets/0.11.1/team-integration-contract.mjs");
...
test("v0.11.1 не записывает состояние ttScore и не изменяет его локальный протокол", () => {
  const app = read("team/assets/0.11.1/app.mjs");
  const integration = read("team/assets/0.11.1/ttscore-integration.mjs");
...
test("v0.11.1 даёт Firebase-editor действие запуска текущей пары в ttScore Team mode", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const adapter = read("team/assets/0.11.1/ttscore-team-adapter.mjs");
  const contract = read("team/assets/0.11.1/team-integration-contract.mjs");
...
test("deployable bundle использует stable root index и Team assets 0.11.1", () => {
  assert.equal(existsSync(new URL("ttscore_0.5.1.html", root)), true);
  assert.equal(existsSync(new URL("team/assets/0.11.1/ttscore-team-adapter.mjs", root)), true);
  const app = read("team/assets/0.11.1/app.mjs");
  const ttScore = read("ttscore_0.5.1.html");
  assert.match(app, /new URL\("\.\.\/index\.html", location\.href\)/);
...
test('RC2 stabilization: ttScore 0.5.1 runtime adapter умеет reconciliate editor-first finished result с backup reportUrl', () => {
  const html = read('ttscore_0.5.1.html');
  ...
  const contract011 = read('team/assets/0.11.1/team-integration-contract.mjs');
  const adapter011 = read('team/assets/0.11.1/ttscore-team-adapter.mjs');
...
test("public list визуально группирует встречи по teamSize без подписей туров", () => {
  const html = read("team/ttscore_team_0.11.1.html");
  const app = read("team/assets/0.11.1/app.mjs");
  const css = read("team/assets/0.11.1/styles.css");

...
import { canOpenCurrentMatchFromView, shouldShowCurrentOperationalLiveLinks, shouldShowPermanentTeamLiveInView } from "./view-state.mjs";
...
const elements = Object.fromEntries([
  ...
  "view-auth-actions", "view-auth-open", "view-auth-sign-out", "view-auth-dialog", "view-auth-form",
  "view-auth-email", "view-auth-password", "view-auth-submit", "view-auth-cancel", "view-auth-error",
  "view-live-links", "view-live-scoreboard", "view-live-report"
...
let currentRenderSource = "firebase";
...
function firebaseAuthEnabled() {
  return firebasePublishingEnabled() || request?.mode === "view";
}

function renderViewAuthState() {
  if (!elements.view_auth_actions) return;
  const isView = request?.mode === "view";
  elements.view_auth_actions.hidden = !isView || !firebaseAuthReady;
  if (!isView) return;
  elements.view_auth_open.hidden = Boolean(firebaseUser);
  elements.view_auth_sign_out.hidden = !firebaseUser;
  if (firebaseUser && elements.view_auth_dialog?.open) elements.view_auth_dialog.close();
}

function rerenderLoadedView() {
  if (request?.mode !== "view" || !rawTeamMatch) return;
  const source = currentRenderSource;
  render(prepareTeamMatch(rawTeamMatch), { source });
}
...
async function initializeFirebaseAuth() {
  ...
  try {
    ...
    firebaseAuthUnsubscribe = await observeFirebaseAuth(user => {
      ...
      renderFirebaseAuthState();
      if (request?.mode === "view") rerenderLoadedView();
      else requestTtScoreAutomation();
    });
  } catch (error) {
    firebaseAuthReady = true;
    renderFirebaseAuthState();
...
function showViewAuthError(error) {
  const code = error?.code ?? "";
  const messages = {
    "auth/invalid-credential": "Неверный email или пароль Firebase.",
    "auth/invalid-email": "Некорректный email Firebase.",
  ...
  elements.view_auth_error.textContent = messages[code] ?? (error instanceof Error ? error.message : String(error));
  elements.view_auth_error.hidden = false;
}

function openViewAuthDialog() {
  if (request?.mode !== "view" || firebaseUser || !firebaseAuthReady) return;
  elements.view_auth_error.hidden = true;
  elements.view_auth_dialog.showModal();
  elements.view_auth_email.focus();
}

async function handleViewFirebaseSignIn(event) {
  event.preventDefault();
  elements.view_auth_error.hidden = true;
  elements.view_auth_submit.disabled = true;
  try {
    await signInFirebaseEditor(elements.view_auth_email.value, elements.view_auth_password.value);
    elements.view_auth_password.value = "";
  } catch (error) {
    showViewAuthError(error);
  } finally {
    elements.view_auth_submit.disabled = false;
  }
}

async function handleViewFirebaseSignOut() {
  elements.view_auth_error.hidden = true;
  try {
    await signOutFirebaseEditor();
  } catch (error) {
...
function renderIndividualMatch(match, teamMatch, { source = "firebase" } = {}) {
  ...
  if (match.status === "finished") {
    ...
    if (shouldShowCurrentOperationalLiveLinks(request?.mode)) {
    ...
    if (canOpenCurrentMatchFromView({
      mode: request?.mode,
      source,
      authenticated: Boolean(firebaseUser),
      lifecycle: teamLifecycle(teamMatch),
      currentMatch: match
    })) {
      result.append(link("Открыть в ttScore", ttScoreTeamLaunchUrl(teamMatch), "button button--primary"));
    }
...
function renderViewPermanentTeamLiveLinks(teamMatchId, source) {
  if (!elements.view_live_links) return;
  const visible = shouldShowPermanentTeamLiveInView({ mode: request?.mode, source });
  elements.view_live_links.hidden = !visible;
  if (!visible) return;
  const urls = permanentTeamLiveUrls(teamMatchId);
  elements.view_live_scoreboard.href = urls.scoreboard;
  elements.view_live_report.href = urls.report;
}
...
function render(teamMatch, { source = "firebase" } = {}) {
  ...
  elements.individual_matches.replaceChildren(...teamMatch.individualMatches.map(match => renderIndividualMatch(match, teamMatch, { source })));
  renderViewPermanentTeamLiveLinks(teamMatch.id, source);
...
elements.view_auth_open.addEventListener("click", openViewAuthDialog);
elements.view_auth_form.addEventListener("submit", handleViewFirebaseSignIn);
elements.view_auth_sign_out.addEventListener("click", handleViewFirebaseSignOut);
elements.view_auth_cancel.addEventListener("click", () => elements.view_auth_dialog.close());
